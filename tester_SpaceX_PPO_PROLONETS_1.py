...
import gym_spacex.envs.SpaceX_PPO_PROLONETS_env1
...
def process():
        ...
        while True:
            ...
            for i in range(0, total_episodes):
                ...
                obs1, r_t, terminal, info = env.step(action, actor_num, current_episode)
                ...
                if total_steps >=10:
                    if obs1[0] == 200 and obs1[1] == 12 and obs1[2] == 30 and obs1[6] == -10 and obs1[7] == -20 and obs1[8] == 15:
                        terminal = True
                ...
                if terminal:
                    ...
                    print('ID :' + ', global episode :' + str(current_episode) + ', episode length :' + str(episode_length) + ', total reward :' + str(total_rewards))
                    ...
                    summary_writer.add_scalar('PPO_Positions/Final X Position', float(states_buf[-2][0]), current_episode)
                    summary_writer.add_scalar('PPO_Positions/Final Y Position', float(states_buf[-2][1]), current_episode)
                    summary_writer.add_scalar('PPO_Positions/Final Z Position', float(states_buf[-2][2]), current_episode)
                    summary_writer.add_scalar('PPO_Rotations/Final Yaw Position', float(states_buf[-2][6]), current_episode)
                    summary_writer.add_scalar('PPO_Rotations/Final Pitch Position', float(states_buf[-2][7]), current_episode)
                    summary_writer.add_scalar('PPO_Rotations/Final Roll Position', float(states_buf[-2][8]), current_episode)
...
env = gym.make("SpaceX_PPO_PROLONETS-v1")
global_episodes = tf.Variable(0, dtype=tf.int32, name='global_episodes', trainable=False)
